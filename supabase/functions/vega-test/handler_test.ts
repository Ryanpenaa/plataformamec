...
Deno.test('rejects requests without the webhook secret', async () => {
  ...
  const response = await handler(new Request('https://example.invalid/vega-test', {
    method: 'POST',
...
Deno.test('stores a valid object payload without forwarding the secret', async () => {
  ...
  assert(storedRequest?.body === JSON.stringify({ payload: { event: 'purchase.approved', transaction_id: 'fake-001' } }), 'expected the payload to be wrapped for storage');
...
Deno.test('rejects arrays and oversized bodies', async () => {
  const handler = makeHandler(settings);
  const arrayResponse = await handler(new Request('https://example.invalid/vega-test', {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'x-vega-test-secret': settings.secret },
    body: '[]',
  }));
  const oversizedResponse = await handler(new Request('https://example.invalid/vega-test', {
    method: 'POST',
    headers: { 'content-type': 'application/json', 'x-vega-test-secret': settings.secret },
