...
export type Course = {
  ...
  title: string;
  subtitle: string;
  modules: { order: number; title: string; lessons: Lesson[] }[];
};
...
export function buildAcademyData(
  studentName: string,
  entitlements: string[],
  developmentPreview: boolean,
): AcademyData {
  ...
  return {
    ...
    courses: developmentPreview
      ? courses
      : courses.filter((course) => allowedCourses.has(course.id)),
