...
const response = (status: number, body: object) => Response.json(body, { status, headers: { "Cache-Control": "no-store" } });
...
export function makeVegaAccessHandler(settings: Settings) {
  return async (request: Request) => {
    ...
    if (!settings.secret || settings.secret.length < 32) return response(503, { error: "Receiver not configured" });
    ...
    if (!request.headers.get("content-type")?.toLowerCase().includes("application/json")) return response(415, { error: "JSON required" });
    const raw = await request.text();
    if (new TextEncoder().encode(raw).byteLength > 262144) return response(413, { error: "Body too large" });

    let payload: unknown;
    try { payload = JSON.parse(raw); } catch { return response(400, { error: "Invalid JSON" }); }
    let event: NormalizedVegaEvent;
    try { event = normalizeVegaPayload(payload); } catch (error) {
      return response(400, { error: error instanceof Error ? error.message : "Invalid payload" });
    }
    if (event.testMode) return response(200, { received: true, processed: false, reason: "test_mode" });
    if (!settings.enabled) return response(202, { received: true, processed: false, reason: "disabled" });
